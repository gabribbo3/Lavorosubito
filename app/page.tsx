...
function statusLabel(status: string) {
  if (status === 'aperta') return '🔴 RICERCA IN CORSO';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`lavorosubito-mvp-${user.id}`)
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  async function loadProfessional() {
    const [
      ...
      supabase.rpc('my_professional_identity'),
      supabase.rpc('my_professional_categories'),
      supabase.rpc('my_professional_setup_status'),
      supabase.rpc('my_professional_availability')
    ]);
